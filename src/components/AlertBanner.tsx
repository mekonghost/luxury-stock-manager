    ...
    <AnimatePresence>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
              <p className="font-black uppercase tracking-widest text-[10px] sm:text-xs">
                {outOfStockItems.length > 0 ? 'Critical Alert' : 'Stock Warning'}
              </p>
              <div className="w-1 h-1 rounded-full bg-white/40 hidden sm:block" />
              <p className="font-bold text-sm">
                {outOfStockItems.length > 0 
                  ? `${outOfStockItems.length} items exhausted`
          ...
          >
            <span className="text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Dismiss</span>
            <X size={18} />
          </button>
        </div>
      </motion.div>
